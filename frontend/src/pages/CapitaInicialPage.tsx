...
import { enviarCapitaInicial, xmlToBase64 } from '../services/capitaInicialApi'
import type { CapitaInicialResponse } from '../services/capitaInicialApi'
import SisproLoginModal from '../components/SisproLoginModal'

interface CapitaInicialUploadProps {
  xmlFile: File | null
  ...
  return (
    <div className="bg-white rounded-xl shadow-lg border border-gray-200 p-8 transition-all duration-200">
      <h2 className="text-xl font-bold text-gray-900 mb-6 flex items-center gap-2">
        ...
        Cargar Archivo de Factura (Capita Inicial)
      </h2>
      ...
      >
        ...
        {xmlFile ? (
          <div className="flex flex-col items-center">
            ...
            <p className="text-lg font-semibold text-gray-900">{xmlFile.name}</p>
      ...
      <div className="mt-6 p-4 bg-blue-50/70 border border-blue-100 rounded-lg">
        <p className="text-sm text-blue-800 leading-relaxed">
          <strong>Nota:</strong> Capita Inicial solo requiere el archivo XML de la factura.
          No lleva archivos RIPS asociados. En el envío al Ministerio, la sección de RIPS se configurará automáticamente como <code>null</code>.
        </p>
      </div>
      ...
      <button
        ...
        className="mt-6 w-full py-3 px-4 bg-teal-500 hover:bg-teal-600 disabled:bg-gray-300 disabled:cursor-not-allowed text-white font-medium rounded-lg transition-colors flex items-center justify-center gap-2"
      >
        {loading ? (
          <>
...
function PayloadPreview({ xmlBase64, onSubmit, onCancel, isSubmitting }: PayloadPreviewProps) {
  const [showPayload, setShowPayload] = useState(true)

  // El payload estructurado tal como se envía al ministerio
  const fullPayload = {
    rips: null,
    xmlFevFile: xmlBase64 ? `${xmlBase64.substring(0, 80)}... [truncado para vista]` : ''
  }
    ...
    <div className="bg-white rounded-xl shadow-lg border border-gray-200 p-8">
        ...
        <div className="border border-gray-200 rounded-lg overflow-hidden">
          ...
          {showPayload && (
            <div className="p-4 bg-gray-950 overflow-auto max-h-96 rounded-b-lg">
              <pre className="text-sm font-mono text-teal-400">
      ...
      <div className="mt-8 flex gap-4">
        ...
        <button
          ...
          className="flex-1 py-3 px-4 bg-teal-500 hover:bg-teal-600 disabled:bg-gray-300 text-white font-medium rounded-lg transition-colors flex items-center justify-center gap-2"
        >
          {isSubmitting ? (
            <>
              <div className="animate-spin rounded-full h-5 w-5 border-b-2 border-white" />
...
interface ValidationResultsProps {
  result: CapitaInicialResponse
  onRetry: () => void
  onClose: () => void
}

function CapitaInicialResults({ result, onRetry, onClose }: ValidationResultsProps) {
    ...
    <div className="bg-white rounded-xl shadow-lg border border-gray-200 p-8">
      <div className="flex items-center gap-3 mb-6">
        ...
        <div>
          ...
          <p className="text-gray-600 mt-1">
            {result.success
              ? 'El paquete de Capita Inicial fue validado correctamente'
      ...
      <div className="mt-8 flex gap-4 flex-wrap">
        ...
        {!result.success && (
          <button
            onClick={onRetry}
            className="flex-1 py-3 px-4 bg-teal-500 hover:bg-teal-600 text-white font-medium rounded-lg transition-colors"
          >
            Reintentar
          </button>
        )}
...
function CapitaInicialContent() {
  ...
  const [validationResult, setValidationResult] = useState<CapitaInicialResponse | null>(null)
  const [isSubmitting, setIsSubmitting] = useState(false)

  const fileInputRef = useRef<HTMLInputElement>(null)
  const canSubmit = xmlFile !== null
  ...
  const handleProcess = async () => {
    ...
    try {
      ...
      const base64String = xmlToBase64(text)
      setXmlBase64(base64String)
      setStep('preview')
    } catch (err) {
      setError(err instanceof Error ? err.message : 'Error al procesar el archivo XML')
    } finally {
      setLoading(false)
    }
  }
  ...
  const submitToMinisterio = async (tokenOverride?: string) => {
    ...
    try {
      ...
      const response = await enviarCapitaInicial(payload, tokenToUse)
      ...
      <div className="max-w-4xl mx-auto">
        <div className="relative mb-8">
          ...
          <p className="text-gray-500 text-center mt-2">
            Validación de Capita Inicial ante el Ministerio de Salud
          </p>
        </div>
...
export default function CapitaInicialPage() {
  return <CapitaInicialContent />
}
