...
import { enviarCapitaPeriodo, fileToBase64 } from '../services/capitaApi'
...
function CapitaFileUpload({ xmlFile, ripsFile, onXmlSelect, onRipsSelect, onProcess, loading, canSubmit, error }: CapitaFileUploadProps) {
  ...
  return (
    <div className="bg-white rounded-xl shadow-md border border-gray-200 p-6">
      <h2 className="text-xl font-semibold text-gray-900 mb-4 flex items-center gap-2">
        ...
        Cargar Archivos Capita Periodo
      </h2>
        ...
        <div>
          ...
          <div className="border-2 border-dashed border-gray-300 rounded-lg p-6 text-center hover:border-emerald-400 transition-colors">
            <input
              ...
              id="rips-upload"
            />
            <label htmlFor="rips-upload" className="cursor-pointer">
              <FileText size={32} className="mx-auto text-gray-400 mb-2" />
              <span className="text-sm text-gray-600">
      ...
      <button
        ...
        className="mt-6 w-full py-3 px-4 bg-emerald-500 hover:bg-emerald-600 disabled:bg-gray-300 disabled:cursor-not-allowed text-white font-medium rounded-lg transition-colors flex items-center justify-center gap-2"
      >
        {loading ? (
          <>
    ...
    <div className="bg-white rounded-xl shadow-md border border-gray-200 p-6">
      ...
      <div className="mt-6 flex gap-3">
        ...
        <button
          ...
          className="flex-1 py-3 px-4 bg-emerald-500 hover:bg-emerald-600 disabled:bg-gray-300 text-white font-medium rounded-lg transition-colors flex items-center justify-center gap-2"
        >
          {isSubmitting ? (
            <>
              <div className="animate-spin rounded-full h-5 w-5 border-b-2 border-white" />
...
interface ValidationResultsProps {
  result: CapitaPeriodoResponse
  onRetry: () => void
  onClose: () => void
}

function CapitaValidationResults({ result, onRetry, onClose }: ValidationResultsProps) {
  ...
  const handleDownloadResponse = () => {
    ...
    // Generar nombre del archivo con el CUV si está disponible
    let filename = 'CUV_Capita.json'
    if (result.codigo_unico_validacion) {
      const cuvShort = result.codigo_unico_validacion.substring(0, 8)
      filename = `CUV_Capita_${cuvShort}.json`
    }
  ...
  // Agrupar notificaciones por código para mostrar resumen
    ...
    <div className="bg-white rounded-xl shadow-md border border-gray-200 p-6">
      <div className="flex items-center gap-3 mb-6">
        ...
        <div>
          ...
          <p className="text-gray-600">
            {result.success
              ? 'El paquete fue validado correctamente'
      ...
      <div className="flex gap-3 flex-wrap">
        ...
        {!result.success && (
          <button
            onClick={onRetry}
            className="flex-1 py-3 px-4 bg-emerald-500 hover:bg-emerald-600 text-white font-medium rounded-lg transition-colors"
          >
            Reintentar
          </button>
        )}
...
function CapitaPeriodoContent() {
  ...
  const [validationResult, setValidationResult] = useState<CapitaPeriodoResponse | null>(null)
  ...
  const submitToMinisterio = async (tokenOverride?: string) => {
    ...
    try {
      ...
      const response = await enviarCapitaPeriodo(payload, tokenToUse)
  ...
  const handleAplicarCorrecciones = async (cambios: CambioAprobado[]) => {
    ...
    try {
      ...
      // Actualizar datos con archivos corregidos
      ...
      // Volver a pantalla de previsualización
      ...
      <div className="max-w-4xl mx-auto">
        ...
        <div className="relative mb-8">
          ...
          <p className="text-gray-600 text-center">
            Validación de Capita Periodo ante el Ministerio de Salud
          </p>
        </div>
...
export default function CapitaPeriodoPage() {
  return <CapitaPeriodoContent />
}
