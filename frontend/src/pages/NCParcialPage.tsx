...
import { enviarNCParcial, fileToBase64 } from '../services/ncParcialApi'
import type { NCParcialResponse } from '../services/ncParcialApi'
import SisproLoginModal from '../components/SisproLoginModal'
import CorreccionPanel from '../components/CorreccionPanel'

interface NCParcialFileUploadProps {
  xmlFile: File | null
  ripsFile: File | null
...
function NCParcialFileUpload({ xmlFile, ripsFile, onXmlSelect, onRipsSelect, onProcess, loading, canSubmit, error }: NCParcialFileUploadProps) {
  ...
  return (
    <div className="bg-white rounded-xl shadow-md border border-gray-200 p-6">
      ...
      <div className="grid md:grid-cols-2 gap-6">
        <div>
          ...
          <div className="border-2 border-dashed border-gray-300 rounded-lg p-6 text-center hover:border-sky-400 transition-colors">
            <input type="file" accept=".xml" onChange={handleXmlChange} className="hidden" id="ncp-xml-upload" />
            <label htmlFor="ncp-xml-upload" className="cursor-pointer">
          ...
          {xmlFile && (
            <p className="mt-2 text-sm text-sky-600 flex items-center gap-1">
              <CheckCircle size={14} />
        ...
        <div>
          ...
          <div className="border-2 border-dashed border-gray-300 rounded-lg p-6 text-center hover:border-sky-400 transition-colors">
            <input type="file" accept=".json" onChange={handleRipsChange} className="hidden" id="ncp-rips-upload" />
            <label htmlFor="ncp-rips-upload" className="cursor-pointer">
              <FileText size={32} className="mx-auto text-gray-400 mb-2" />
              <span className="text-sm text-gray-600">
      ...
      <button
        ...
        className="mt-6 w-full py-3 px-4 bg-sky-500 hover:bg-sky-600 disabled:bg-gray-300 disabled:cursor-not-allowed text-white font-medium rounded-lg transition-colors flex items-center justify-center gap-2"
      >
        {loading ? (
          <>
...
function PayloadPreview({ ripsData, xmlBase64, onSubmit, onCancel, isSubmitting }: PayloadPreviewProps) {
  ...
  return (
    <div className="bg-white rounded-xl shadow-md border border-gray-200 p-6">
      <h2 className="text-xl font-semibold text-gray-900 mb-4 flex items-center gap-2">
        <Eye size={24} className="text-sky-500" />
      ...
      <div className="mt-6 flex gap-3">
        ...
        <button
          ...
          className="flex-1 py-3 px-4 bg-sky-500 hover:bg-sky-600 disabled:bg-gray-300 text-white font-medium rounded-lg transition-colors flex items-center justify-center gap-2"
        >
          {isSubmitting ? (
            <>
              <div className="animate-spin rounded-full h-5 w-5 border-b-2 border-white" />
...
interface ValidationResultsProps {
  result: NCParcialResponse
  onRetry: () => void
  onClose: () => void
}

function NCParcialValidationResults({ result, onRetry, onClose }: ValidationResultsProps) {
  ...
  return (
    <div className="bg-white rounded-xl shadow-md border border-gray-200 p-6">
      <div className="flex items-center gap-3 mb-6">
        ...
        <div>
          ...
          <p className="text-gray-600">
            {result.success ? 'La NC Parcial fue validada correctamente' : 'Se encontraron errores en la validación'}
          </p>
        </div>
      </div>
        ...
        <div className="mb-6 border border-yellow-200 rounded-lg overflow-hidden">
          ...
          {notifExpanded ? (
            ...
            <div className="p-4 bg-yellow-50/50">
              ...
              <p className="text-xs text-yellow-600 mt-1">Haz clic en el encabezado para expandir</p>
            </div>
          )}
        </div>
      )}
      ...
      <div className="flex gap-3 flex-wrap">
        ...
        {!result.success && (
          <button onClick={onRetry} className="flex-1 py-3 px-4 bg-sky-500 hover:bg-sky-600 text-white font-medium rounded-lg transition-colors">
            Reintentar
          </button>
        )}
      </div>
...
function NCParcialContent() {
  ...
  const [validationResult, setValidationResult] = useState<NCParcialResponse | null>(null)
  ...
  const submitToMinisterio = async (tokenOverride?: string) => {
    ...
    try {
      ...
      const response = await enviarNCParcial(payload, tokenToUse)
    ...
    <div className="min-h-screen py-8 px-4">
      <div className="max-w-4xl mx-auto">
        <div className="relative mb-8">
          ...
          <h1 className="text-3xl font-bold text-center mb-2">Nota Crédito Parcial</h1>
          <p className="text-gray-600 text-center">Validación de NC Parcial ante el Ministerio de Salud (RIPS + XML)</p>
        </div>
        ...
        {step === 'results' && validationResult && !showCorreccion && (
          <NCParcialValidationResults result={validationResult} onRetry={() => setStep('preview')} onClose={handleReset} />
        )}
...
export default function NCParcialPage() {
  return <NCParcialContent />
}
